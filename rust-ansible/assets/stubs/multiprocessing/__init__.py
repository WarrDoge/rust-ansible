...
This module also patches the real multiprocessing module to ensure
ansible's use of multiprocessing.get_context('fork') works.
"""
...
# Patch the real multiprocessing module so that get_context() returns our stub
def _patch_real_multiprocessing():
    """Patch the real multiprocessing module to use our stubs.

    This is needed because ansible uses multiprocessing.get_context('fork')
    directly instead of our stub module.
    """
    try:
        import multiprocessing as real_mp

        # Store original functions
        _original_get_context = real_mp.get_context
        _original_parent_process = getattr(real_mp, 'parent_process', None)

        # Create a context class that wraps our stub
        class WasiContext:
            """WASI-compatible multiprocessing context."""
            Process = Process
            Queue = queues.Queue
            SimpleQueue = queues.SimpleQueue
            JoinableQueue = queues.JoinableQueue
        ...
        # Replace get_context to return our stub context
        def patched_get_context(method=None):
            # Always return our WASI-compatible context
            return WasiContext()

        real_mp.get_context = patched_get_context

        # Also patch the module-level parent_process
        real_mp.parent_process = lambda: _parent_process

        # Patch multiprocessing.Process too
        real_mp.Process = Process

    except Exception as e:
        # If patching fails, just continue - the stub module is still available
        import sys
        print(f"Warning: Failed to patch real multiprocessing module: {e}", file=sys.stderr)


# Apply patches when this module is imported
_patch_real_multiprocessing()
