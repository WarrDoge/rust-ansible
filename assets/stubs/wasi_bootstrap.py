...
def initialize():
    ...
    Applies OS-level patches immediately, then attempts to apply ansible
    patches if ansible is already importable. If ansible hasn't been
    imported yet, the patches will be applied lazily on first ansible
    import via _deferred_ansible_patches().
    """
    global _initialized
    if _initialized:
        return
    ...
    # Try to apply ansible patches now; if ansible isn't imported yet,
    # install a meta_path hook that applies them on first ansible import.
    try:
        import ansible  # noqa: F401
        apply_ansible_patches()
    except ImportError:
        _install_deferred_ansible_patches()


def _install_deferred_ansible_patches():
    """Install an import hook that applies ansible patches after ansible loads."""
    import importlib.abc
    import importlib.machinery

    class _AnsiblePatchFinder(importlib.abc.MetaPathFinder):
        """One-shot finder that triggers apply_ansible_patches() after ansible.executor loads."""

        def find_module(self, fullname, path=None):
            if fullname == 'ansible.executor.process.worker':
                # Remove ourselves so we only fire once, then apply patches
                # after the real import completes.
                sys.meta_path.remove(self)
                # Schedule patches for after the import finishes
                import atexit
                atexit.register(apply_ansible_patches)
            return None  # We never actually provide a module

    sys.meta_path.insert(0, _AnsiblePatchFinder())
